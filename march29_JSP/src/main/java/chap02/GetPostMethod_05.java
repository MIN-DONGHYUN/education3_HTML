...
/* === 중요 === (암기 부분)
	...
	현재 /JSPServletBegin/05_getPostMethod.do 이므로
	http://localhost:9090/JSPServletBegin/05_getPostMethod.do 이 된다. 	
	
	
	== 배치서술자인 web.xml 에 기술하지 않고 @WebServlet 어노테이션을 사용한 예제 ==
	
	http://localhost:9090/JSPServletBegin/05_getPostMethod.do 을 처리해주는 서블릿은 GetPostMethod_05 이다.
*/


@WebServlet("/05_getPostMethod.do")     // @WebServlet("URL경로")   ==> 애도 WAS 를 껏다가 켜야함
